...
export async function POST(request: NextRequest) {
  ...
  if (!event.owner_user_id) {
    ...
    if (updateError) {
      return NextResponse.json(
        { ok: false, error: "Failed to claim event." },
        { status: 500 }
      );
    }
    ...
    if (refreshError || !refreshed) {
      return NextResponse.json(
        { ok: false, error: "Failed to refresh event ownership." },
        { status: 500 }
      );
    }

    if (refreshed.owner_user_id !== authData.user.id) {
      return NextResponse.json(
        { ok: false, error: "Event already claimed" },
        { status: 403 }
      );
    }

    return NextResponse.json({ ok: true, event: refreshed });
  }
